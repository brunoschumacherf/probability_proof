...
gerar_dados <- function(m1=202121316,
                        m2=202121316,
                        m3=202121316,
                        n=2e3)
{
...
gerar_dados_rl <- function(m1=202121316,
                           m2=202121316,
                           m3=202121316,
                           n=10)
{
...
gerar_tdf <- function(m1=202121316,
                      m2=202121316,
                      m3=202121316)
{
  stopifnot(is.numeric(m1) &
...
dad = gerar_dados(m1=202121316, m2=202121316, m3=202121316)
dad_rl = gerar_dados_rl(m1=202121316, m2=202121316, m3=202121316)
tb = gerar_tdf(m1=202121316, m2=202121316, m3=202121316)
...
tabela <- matrix(0, nrow = 1, ncol = 3)
colnames(tabela) <- c("25%", "50%", "75%")

for (j in 1:3) {
  n <- sum(tb$f) * quartis[j]
  i <- 1
  aux <- 0
  ...
  q <- intervalos[i, 1] + ((n - f_a[i - 1]) / tb$f[i]) * 10
  tabela[j] <- q
}
...
for (j in 1:9) {
  ...
  d <- intervalos[i, 1] + ((n - f_a[i - 1]) / tb$f[i]) * 10
  tabela[j] <- d
}
